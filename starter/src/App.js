...
function App() {
  ...
  const updateData = (newShelf, book) => {
    setBookData((currentData) => {
      // Remove the book from all shelves
      const updatedData = Object.keys(currentData).reduce((acc, shelf) => {
        acc[shelf] = currentData[shelf].filter((b) => b.id !== book.id);
        return acc;
      }, {});

      // Add the book to the new shelf (if it's not "none")
      if (newShelf !== "none") {
        updatedData[newShelf] = [...updatedData[newShelf], book];
      }
  ...
  const removeData = (shelf, book) => {
    setBookData((currentData) => ({
      ...currentData,
      [shelf]: currentData[shelf].filter((b) => b.id !== book.id),
    }));
  };
